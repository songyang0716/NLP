...
class encoder(nn.Module):
    ...
    def forward(self, sentence_batch, sequence_length):
        ...
        packed_embeddings = \
            nn.utils.rnn.pack_padded_sequence(sentence_embedding_layer,
                                              sequence_length.cpu(),
                                              batch_first=True,
    ...
    def forward(self, prem_batch, hyp_batch, prem_length, hyp_length):
        ...
        M_premise = \
            ...
            self.WH(torch.matmul(prem_mean_vec,
                                 torch.ones([1,
                                             prem_bilstm_output.shape[1]], device='cuda')).permute(0,2,1))
        ...
        M_hyp = \
            ...
            self.WH(torch.matmul(hyp_mean_vec,
                                 torch.ones([1,
                                             hyp_bilstm_output.shape[1]], device='cuda')).permute(0,2,1))
        # Size of M is like  batch * sequence * embedding
        ...
        second_last_out = torch.cat((weighted_premise,
