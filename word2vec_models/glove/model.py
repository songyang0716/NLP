...
class GloVe(nn.Module):
	...
	def forward(self, input_idx, output_idx):
		...
		output_matrix = torch.sum(torch.mul(input_batch_embed, output_batch_embed), dim=1) + input_batch_bias + output_batch_bias - tensor.log(self.co_oc[input_idx, output_idx])
	...
	def _weight(self, input_idx, output_idx):
		weights = torch.ones(size=len(input_idx))
		weights[self.co_oc[input_idx, output_idx] <= self.x_max] = (self.co_oc[input_idx, output_idx] / self.x_max) ** self.alpha
		return weights
